...
export const Plasma = ({
  ...
  useEffect(() => {
    ...
    const renderer = new Renderer({
      ...
      alpha: true,
      antialias: false,
      dpr: Math.min(window.devicePixelRatio || 1, 1.5) // Limit DPR for performance
    });
    ...
    const setSize = () => {
      const rect = containerRef.current.getBoundingClientRect();
      // Limit maximum resolution for performance
      const maxWidth = Math.min(rect.width, 1920);
      const maxHeight = Math.min(rect.height, 1080);
